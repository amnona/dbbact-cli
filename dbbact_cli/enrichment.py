...
'''
dbbact-cli script
dbbact term enrichment between 2 fasta files
'''
...
from dbbact_cli.dsfdr import dsfdr
...
class DBBact():
	def __init__(self, exp=None, username='', password=''):
	...
	def add_annotations(self, expid, sequences, annotationtype, annotations, submittername='NA', description='', method='na', primerid='V4', agenttype='Calour', private='n'):
		"""
	...
	def enrichment(self, exp_features, features, sequence_annotations, dbbact_annotations, term_type='term', ignore_exp=None, min_appearances=3, fdr_method='dsfdr'):
		...
		bg_features = np.array(list(set(exp_features).difference(set(features))))

		# add all annotations to experiment if not already added
		if term_type == 'term':
			feature_terms = self._get_all_term_counts(exp_features, sequence_annotations, dbbact_annotations, ignore_exp=ignore_exp)
...
def enrichment(group1, group2, output):
	seqs1 = []
	seqs2 = []
	print('loading group 1 file %s' % group1)
	for cseq, chead in iterfastaseqs(group1):
		seqs1.append(cseq)
	print('loaded %s sequences for group 1' % len(seqs1))
	print('loading group 2 file %s' % group2)
	for cseq, chead in iterfastaseqs(group2):
		seqs2.append(cseq)
	print('loaded %s sequences for group 2' % len(seqs2))
	if len(set(seqs1).intersection(set(seqs2)))>0:
		print('WARNING: fasta files are not disjoint. Continuing')
	db = DBBact()
	allseqs = seqs1+seqs2
	print('getting annotations')
	sequence_terms, sequence_annotations, annotations, term_info = db.get_seq_list_fast_annotations(allseqs)
	print('calculating enrichment')
	res = db.enrichment(allseqs, seqs1, sequence_annotations, annotations)
	print('found %d significantly enriched. saving to file %s' % (len(res),output))
	res.to_csv(output, sep='\t', index=False)
	print('done')


def main(argv):
	parser = argparse.ArgumentParser(description='enrichment.py version ' + __version__)
	parser.add_argument('--group1', help='input fasta file for group 1 sequences')
	parser.add_argument('--group2', help='input fasta file for group 2 sequences')
	parser.add_argument('-o','--output', help='output file name')

	args = parser.parse_args(argv)
	enrichment(group1=args.group1, group2=args.group2, output=args.output)
